data_complexaction = [
    ...
    },
    {
		...
		"<b>Set/Disarm Data Bomb:</b>Test to set: Software + LOG [Sleaze] v. (Device Rating x 2).<br>Test to Disarm: Software + INT [Firewall] v. Data Bomb Rating x 2<br>A set Data Bomb is triggered when someone attempts to manipulate a file without using the Data Bomb’s passcode. When a Data Bomb goes off, it causes (Rating)D6 Matrix Damage (resisted normally) to the icon that tripped it, deletes the file (if it was set that way), and then is itself deleted.A Data Bomb can be detected using Matrix Perception. If detected, it can be defused with the Disarm Data Bomb action; A sucessfully disarmed Data Bomb deletes itself. If you fail, the bomb detonates and potentially destroys any file it was attached to (if set to do so).",
