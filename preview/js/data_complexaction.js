data_complexaction = [
    ...
    },
    {
        ...
        description: "A character may perform most Matrix actions with a Complex Action.",
        reference: "SR5, pgs. 164, 244.",
        bullets: [
            "<b>Crash Program:</b> On a successful <i>Cybercombat + LOG [Attack] v. INT + Firewall</i> Test, you overload part of your target’s memory and scramble one of its running programs.<br>You need to specify which program you’re trying to crash; you can learn what your target is running either with a Matrix Perception action or by observing the program in action.<br>If you succeed, the program ends and cannot be restarted until the device it was running on is rebooted.",
	    "<b>Data Spike:</b> On a successful <i>Cybercombat + LOG [Attack] v. INT + Firewall</i> Test, you send harmful instructions to a persona or device, causing Matrix damage to the target. The DV is equal to the rating of your Attack program, with one additional box of damage per net hit, and two additional boxes of damage for each mark you have on the target.",
	    "<b>Set/Disarm Data Bomb:</b> Make a Software + LOG [Sleaze] v. (Device Rating x 2) Test to set a Data Bomb.<br>A set Data Bomb is triggered when someone attempts to manipulate a file without using the Data Bomb’s passcode. When a Data Bomb goes off, it causes (Rating)D6 Matrix Damage (resisted normally) to the icon that tripped it, deletes the file (if it was set that way), and then is itself deleted.<br>A Data Bomb can be detected using Matrix Perception. If detected, it can be defused with the Disarm Data Bomb action; On a successful <i>Software + INT [Firewall] v. Data Bomb Rating x 2</i> the Data Bomb is disarmed. A disarmed Data Bomb deletes itself. On a failure, the bomb detonates and potentially destroys any file it was attached to (if set to do so).",
	    "<b>Brute Force:</b> With a successful <i>Cybercombat + LOG [Attack] v. Willpower + Firewall</i> test, this action allows you to mark a target without obtaining the permissions you would otherwise need. If you succeed in this action, you place one mark. You can have up to a maximum of three marks per icon.<br>If the target can take Matrix damage you may also inflict 1 DV of Matrix damage to the target for every two full net hits, which is resisted with the target’s Device Rating + Firewall.<br>If you try for two marks in one shot, you take a –4 dice pool penalty on the attempt. If you try for three marks in a single swipe, you take a –10 dice pool penalty.<br>You can also use this action to hop to a grid for which you don’t have legitimate access. The defense dice pool in this case is 4 dice for a local grid or 6 dice for a global grid. If you succeed you hop to that grid immediately."
        ]
    },
    {
        title: "Delay Action",
